...
def write_to_slugs_part_dist_impsensors_boundaries(infile, gw, init, initmovetarget, invisibilityset, targets, vel=1, visdist=5,
                                        allowed_states=[], fullvis_states=[], partialvis_states=dict(),
                                        partitionGrid=dict(), belief_safety=0, belief_liveness=0,
    ...
    borderstates = set()
    borderbeliefstates = set()
    # writing env_trans
    file.write('\n[ENV_TRANS]\n')
    for st in tqdm(
            set(allstates) - (set(nonbeliefstates) - set(allowed_states))):  # Only allowed states and belief states
        if st in allowed_states:
            for s in allowed_states:
                stri = "(s = {} /\\ st = {}) -> ".format(s, st)
                beliefset = set()
            ...
            for s in allowed_states:
                ...
                for currbeliefstate in beliefcombstate:
                    beliefstates = beliefstates.union(partitionGrid[currbeliefstate])
                beliefstates = beliefstates - set(targets)  # remove taret positions (no transitions from target positions)
                if len(beliefstates.intersection(borderstates)) > 0:
                    borderbeliefstates.add(st)
                beliefstates_vis = beliefstates.intersection(visstates)
                beliefstates_invis = beliefstates - beliefstates_vis
                ...
                if len(beliefstates) > 0:
                    for sensorvals in sensorcombos:
                        ...
                        for n in sensorvals:
                            stri += "/\\ sensor{} = {} ".format(partialvis_states.keys()[sensorvals.index(n)], n)
                        inactivesensors = [i for i, e in enumerate(sensorvals) if e == 0]
                        activesensordict = {i: set(partialvis_states[i]) for i in partialvis_states if
                                            i not in inactivesensors}

                        if len(activesensordict) == 0:
                            sensedbeliefstates = beliefstates - set.union(*partialvis_states.values())
                        else:
                            sensedbeliefstates = set.intersection(*activesensordict.values()).intersection(
                                beliefstates_invis)
                        stri += ") -> ("
                        beliefset = set()
                        for b in sensedbeliefstates:
                                    ...
                                    if not t in invisibilityset[s]:
                                        ...
                                        try:
                                            t2 = partitionGrid.keys()[
                                                [inv for inv in range(len(partitionGrid.values())) if
                                                 t in partitionGrid.values()[inv]][0]]
                                            beliefset.add(t2)
                                        except:
                                            print t
            ...
            if len(beliefset) > belief_safety:
                ...
                for sensorvals in sensorcombos:
                    ...
                    for x in allowed_states:
                        ...
                        truebelief = beliefset.intersection(invisstates)
                        if len(activesensordict) > 0:
                            truesensedbeliefstates = set.intersection(*activesensordict.values()).intersection(
                                frozenset(truebelief))
                        else:
                            truesensedbeliefstates = truebelief - set.union(*partialvis_states.values())
                        if len(truesensedbeliefstates) > belief_safety:
                            stri += '!('
