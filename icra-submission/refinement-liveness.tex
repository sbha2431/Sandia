...
% we conclude that the abstract counterexample $\counterex_\abstr$ is not concretizable. 
...
$\mathcal D' := \mathcal D$\;
 \ForEach{node $d$ in $\mathcal D$ labelled with $\langle(l_a,B_t),v\rangle$}{
  ...
  \lIf{there is no node in $ D$ labelled with $\langle (l_a',B_t'),v'\rangle$}{add $d'$ labelled $\langle (l_a',B_t'),v'\rangle$}
