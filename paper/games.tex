...
\item $\Vis = \states \times \bool^n \to \bools$ is a function $(\vis_1,\dots,\vis_n)$ where $\vis_i: \states \to \bools$ is a function that maps a state $(l_i,l_t)$ to $\true$ iff \emph{ position $l_t$ is in the area of sight of $l_i$}.
\end{itemize}
