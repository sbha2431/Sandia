...
\begin{itemize}
\item $\states = L_1^a \times L_2^a \dots L_n^a \times L^t$ is the set of states, with $L^a_i$ the set of locations of sensor $i$, and $L^t$ the locations of the target;
\item $s^\init = (l_1^\init\times \dots l_n^\init,l_t^\init)$ is the initial state;
...
\item given $n$ \textit{visibility functions} $(\vis_1,\dots,\vis_n)$ where $\vis_i: L_i^a \times L^t \to \bools$ is a function that maps a state $(l_i,l_t)$ to $\true$ iff \emph{ position $l_t$ is in the area of sight of $l_i$}, $\Vis = \states \to \bools$ is a function that maps to $\true$ if the set $I = \{i: \vis_i(l_i^a,l^t) = \true\}$ is non-empty. Simply, $\Vis$ outputs $\true$ if the target is in view with \emph{at least one} of the sensors.
...
\filldraw[fill=blue!40!white,draw=black]  (-1.25,+0.75) circle (0.2cm);
...
\vspace{-0.1cm}
\caption{A simple surveillance game on a grid arena. Obstacles are shown in red, the sensors (at locations 0 and 4) and the target (at location 18) are coloured in blue and orange respectively.}
