We begin by defining a formal model for describing multi-agent surveillance strategy synthesis problems, in the form of a two-player game between the mobile sensors and a target, in which the sensors have only partial information about the target's location.

\subsection{Surveillance Game Structures}\label{sec:surveillance-games}
We define a \emph{multi-agent surveillance game structure} to be  a tuple $G  = (\states,s^\init,\trans,\Vis)$, with the following components:
\begin{itemize}
\item $\states = L \times L_t$ is the set of states, with $L = L_{1} \times L_{2} \dots L_{n}$ being the set of joint locations of the $n$ sensors, and $L_t$ the locations of the target;
\item $s^\init = (l^{\init},l_t^\init)$ is the initial state;
...
\item given $n$ \textit{visibility functions} $(\vis_1,\dots,\vis_n)$ where $\vis_i: L_{i} \times L_t \to \bools$ is a function that maps a state $(l_{i},l_t)$ to $\true$ iff \emph{ position $l_t$ is in the area of sight of $l_i$}, $\Vis = \states \to \bools$ is a function that maps to $\true$ if the set $\mathcal{I} = \{i \mid \vis_i(l_i,l_t) = \true\}$ is non-empty. Informally, $\Vis$ outputs $\true$ if the target is in view of \emph{at least one} of the sensors.
...
\vspace{-0.1cm}
\caption{A simple surveillance game on a grid arena. Obstacles are shown in red. There are two sensors (at locations 0 and 4) coloured in blue and green respectively and the target (at location 18) is orange.}
\label{fig:simple-surveillance-game}
\vspace{-.7cm}
\end{figure}


The transition relation $T$ encodes the one-step move of the target and the $n$ sensors, where the target moves first and the sensors move jointly second. For a state $(l,l^t)$ we denote $\succs_t(l,l^t)$ as the set of successor locations of the target:

$\succs_t(l,l_{t}) = \{l_{t}' \in L_t \mid \exists l'.\ ((l,l_t),(l',l_t')) \in T\}$.

We extend $\succs_t$ to sets of locations of the target by stipulating that the set $\post(l,L)$ consists of all possible successor locations of the target for states in $\{l\} \times L$. Formally, let $\post(l_a, L) = \bigcup_{l_t \in L_t}\succs_t(l,l_t)$.
...
\begin{itemize}
\item $\states_\belief = L \times \beliefs$ is the set of states, with $L_a$ the set of locations of the sensors, and $\beliefs$ the set of \emph{belief sets} describing information about the location of the target;
\item $s^\init_\belief = (l^\init,\{l_t^\init\})$ is the initial state;
...
\end{itemize}
\end{itemize}
Condition (1) captures the successor locations of the target that can be observed from one of the sensors' current location $l$. Condition (2) corresponds to the belief set consisting of \emph{all possible successor locations of the target not visible from $l$}. 
%\begin{itemize}
%\item[(1)] $B_t' = \{l_t'\}$ for some $l_t'$ such that $\vis(l_a,l_t') = \true$ and
%there exists $l_t \in B_t$ with $((l_a,l_t),(l_a',l_t')) \in \trans$;
...
Based on  $T_\belief$, we can define the functions $\succs_t : \states_\belief \to \mathcal{P}(\beliefs)$ and  $\succs : \states_\belief \times \beliefs \to \mathcal{P}(L_a)$ similarly to the corresponding functions defined for $G$. 
...
A \emph{strategy for the sensors in $G_\belief$} is a function $f_s : S^+ \times \beliefs \to S$ such that $f_s(\pi\cdot s,B_t) = (l_a',B_t')$ implies $B_t' = B_t$ and $l_a' \in \succs_a(s,B_t)$ for every $\pi \in \states_\belief^*$, $s \in \states_\belief$ and $B_t \in \beliefs$. Intuitively, a strategy for the sensors suggests a move based on the observed history of the play and the current belief about the target's position.
...
A \emph{global surveillance game} is a pair $(G,\varphi)$, where $G$ is a surveillance game structure and $\varphi$ is a surveillance objective. A \emph{winning strategy for the sensors for $(G,\varphi)$} is a joint strategy $f_s$ for the sensors in the corresponding belief-set game structure $G_\belief$ such that for every strategy $f_t$ for the target in $G_\belief$ it holds that $\outcome(G_\belief,f,f_t) \models \varphi$. Analogously, a \emph{winning strategy for the target for $(G,\varphi)$} is a strategy $f_t$ such that, for every strategy $f_a$ for the sensors in $G_\belief$, it holds that $\outcome(G_\belief,f_a,f_t) \not\models \varphi$.

{\bf Global surveillance synthesis problem:} Given a surveillance game $(G,\varphi)$, compute a joint winning strategy for the agent for $(G,\varphi)$, or determine that such a strategy does not exist.

There are two sources of computational complexity in this synthesis problem. Firstly, the powerset construction necessary to construct the belief game causes an exponential increase in the state space. Methods such as constructing an \emph{abstract belief game} to mitigate the state space explosion have been proposed \cite{}. However, in the worst case that method can still result in full state space explosion occurring. Secondly, the addition of multiple sensors can help enforce stronger surveillance objectives, but this comes at the cost of exponentially increasing the state space for each sensor introduced. In the next section we propose a method of distributing the currently centralized global surveillance game into smaller \emph{surveillance subgames}. 