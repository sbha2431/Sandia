...
\Do{$\mathcal D \neq \mathcal D'$}{
$\mathcal D' := \mathcal D$\;
 \ForEach{node $d$ in $\mathcal D$ annotated with $\langle(l_a,B_t),v\rangle$}{
  \ForEach{child $v'$ of $v$ in $\counterex_\abstr$ labelled $(l_a',A_t')$}{
  $B_t' := \post(B_t)\cap\gamma(A_t')$\;
  \eIf{there is a node $d' \in D$ annotated with $(l_a',B_t)$}{add an edge $(d,d')$ to $E$}
  {add a node $d'$ annotated $(l_a',B_t)$ to $D$\;
