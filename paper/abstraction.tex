For a partitioning $\part = \{Q_i\}_{i=1}^n$ of the set of target positions $L_t$ in a game structure $G$ we define the function $\alpha_\part : L_t \to \part$ by letting $\alpha(l) = Q$ for the unique $Q \in \part$ with $l \in Q$. We denote also with $\alpha_{\part} : \mathcal{P}(L_t) \to \mathcal{P}(\part)$ the \emph{abstraction function} defined by $\alpha_{\part}(L) = \{\alpha_\part(l) \mid l \in L\}$.
...
\end{cases}
\]
Given a surveillance game structure $G  = (\states,s^\init,\trans,\vis)$ and a partitioning $\part = \{Q_i\}_{i=1}^n$ of the set $L_t$, we define the \emph{abstraction of $G$ w.r.t.\ $\part$} to be the game structure 
