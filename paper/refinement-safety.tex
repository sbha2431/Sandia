\todo{In order to determine if an abstract counterexample tree is concretizable, we propagate the precise belief-sets corresponding to the abstract moves of the target...}
...
\todo{When an abstract counterexample tree is determined unconcretizable, the returned path corresponds to a sequence of moves to agent, for which the size of the concrete belief does not actually exceed the threshold as in the abstract counterexample. We then refine the abstraction in order to precisely capture this information and thus eliminate the abstract counterexample.}

\subsubsection{Backward partition splitting}
Consider a path $\pi_\abstr = v_0,\ldots, v_n$ in $\counterex_\abstr$ where $v_0$ is the root node and $v_n$ is a leaf. For each node $v_i$, let $(l_a^i,A_t^i) $ be the abstract state labelling $v_i$ in $\counterex_\abstr$, and let $(l_a^i,B_t^i)$ be the  belief set with which the node was annotated by the the counterexample analysis procedure. We consider the case when $(l_a^n,B_t^n) \models p_k$, that is, $|\{l_t \in B_t^n \mid \vis(l_a,l_t) = \false\}| \leq k$.
...
\begin{example}
We continue with the unconcretizable abstract counterexample tree from Example~\ref{ex:simple-safety-unconcretizable}. We illustrate the refinement procedure for the path $v_0,v_1,v_3$. For node $v_3$, we split $Q_1$ and $Q_2$ using the set $B = \{30,32,38\}$, obtaining the sets $Q_1' = Q_1 \cap \{30,32,38\} = \{30\}$, $Q_2' = Q_1\setminus\{30\}$, $Q_3' = Q_2 \cap \{30,32,38\} = \{32,38\}$ and $Q_4' = Q_2 \setminus \{32,38\}$. We thus obtain a new partition $\part_{v_3} \preceq \part$. In order to propagate the refinement backwards (to ensure eliminating $\counterex_\abstr$) we compute the set of locations from which the target can move to a location in $Q_2'$ or $Q_4'$ that is not visible from location $10$. In this case, these are just the locations $32$ and $38$, which have already been separated from $Q_2$, so here backward propagation does not require further splitting.\qed
\end{example}
