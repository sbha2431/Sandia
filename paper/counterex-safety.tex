A winning strategy for the target in the abstract game $(G_\abstr,\LTLglobally p_k)$ can be naturally represented as a tree. 
...
\todo{since the process of abstraction increases the power of the target and gives less power to the agent, an abstract counterexample does not necessarily correspond to a winning strategy for the target in the belief-set game}
...
\input{figs/simple-safety-counterex-abstr.tex}
}
\hfill
\subfloat[Concrete counterexample tree\label{fig:simple-safety-counterex-concr}]{
\input{figs/simple-safety-counterex-concr.tex}
}
